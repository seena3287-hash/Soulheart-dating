...
export const metadata = {
  ...
  description: 'Browse verified profiles across India. Connect in English, Hindi, Kannada, Telugu and more on SoulHeartDatingApp.'
};

export default async function ProfilesPage({ searchParams }) {
  const params = await searchParams;
  const page = Math.min(Math.max(parseInt(params?.page || '1', 10) || 1, 1), 5);
    ...
    <div className="relative min-h-screen bg-[#0a0810] text-white">
        ...
        <div className="container mx-auto px-4 py-14 md:px-8 md:py-20">
          <div className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-5 max-w-2xl text-base text-white/70 md:text-lg">
              Meet, chat and connect with premium verified profiles from every corner of India. Real people. Real conversations. Real chemistry.
            </p>
            <div className="mt-8">
              <HeroCTA />
            </div>
      ...
      <section className="relative container mx-auto px-4 pb-16 md:px-8">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="font-display text-2xl font-semibold md:text-3xl">Featured Profiles</h2>
            <p className="mt-1 text-sm text-white/60">Page {page} of 5 · 24 profiles per page</p>
          </div>
          <div className="flex items-center gap-2 text-xs text-white/60">
            <span className="h-2 w-2 rounded-full bg-emerald-400" /> Green dot = Online now
          </div>
        </div>
        ...
        <div className="mt-14">
          <Pagination current={page} total={5} />
        </div>
      </section>
  ...
  return (
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {Array.from({ length: 24 }).map((_, i) => (
        <div key={i} className="animate-pulse rounded-3xl border border-white/10 bg-white/[0.03] p-4">
